...
const Servers = () => {
  ...
  const [filter, setFilter] = useState('all'); // all | recommended | favorites
  ...
  const renderServer = (item) => {
    ...
    return (
      ...
      >
        ...
        <TouchableOpacity onPress={() => toggleFavorite(item.id)} style={{ marginLeft: 10, padding: 4 }}>
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: '#f5f7fa' }}>
      ...
      <ScrollView contentContainerStyle={{ paddingHorizontal: 20, paddingBottom: 30 }} showsVerticalScrollIndicator={false}>
        {filtered.length === 0
          ...
          : filtered.map(item => renderServer(item))
        }
      </ScrollView>
